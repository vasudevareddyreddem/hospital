...
class Resources extends In_frontend {
	...
	{	
		if($this->session->userdata('userdetails'))
		{
				if($admindetails['role_id']=3){
					...
					$data['vital_list']= $this->Resources_model->get_vitals_list($patient_id);
					//echo '<pre>';print_r($data);exit;
	...
	// doctor timings
	public function get_doctors_times_list(){
		$post=$this->input->post();
		$doctor_list=$this->Resources_model->get_doctors_times_list($post['doctor_id']);
